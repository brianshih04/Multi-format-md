...
- 執行摘要包含掃描、略過、成功、失敗、VLM 圖片請求數及 AI 整理請求數。
...
- Windows 11 實際批次驗證使用 12 份文件（3 個 DOCX、9 個 XLSX）：首次全部成功並產生 4 次 AI 文字請求（3 次 DOCX 段落、1 次 XLSX 空白工作表標記），第二次全部由 Manifest 略過且不再呼叫 API。
- 實際企業文件、轉換結果、API Key、內部路徑及可識別個人的檔名不得提交至公開儲存庫。
